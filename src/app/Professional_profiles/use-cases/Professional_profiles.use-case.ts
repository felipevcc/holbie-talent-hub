...
export const ProfilePost: RequestHandler = async (req: Request, res: Response) => {
  try {
    const { is_user, headline, about_me, location, job_name, kind_job, job_type, salary_min, salary_max, readme, profile } = req.body;

    const sqlQuery = await query('professional_profiles')
      .insert({ is_user, headline, about_me, location, job_name, kind_job, job_type, salary_min, salary_max, readme, profile });
...
export const ProfilePut: RequestHandler = async (req: Request, res: Response) => {
  try {
    const { profile_id } = req.params;
    const { headline, about_me, location, job_name, kind_job, job_type, salary_min, salary_max, readme, profile } = req.body;

    const sqlQuery = await query('professional_profiles')
      .where('profile_id', profile_id)
      .update({ headline, about_me, location, job_name, kind_job, job_type, salary_min, salary_max, readme, profile });
